...
contactForm.addEventListener("submit", function (event) {
    ...
    formSuccess.textContent = "Opening your email application...";
...
fetch("https://dev.to/api/articles/latest?per_page=30")
    .then((response) => {
        ...
        technologyArticles.forEach((article) => {
            ...
            html += `
                <article class="article-item">
                    <h3>${article.title}</h3>
                    <p>
                        ${article.description || "Technology article from the DEV Community."}
                    </p>
